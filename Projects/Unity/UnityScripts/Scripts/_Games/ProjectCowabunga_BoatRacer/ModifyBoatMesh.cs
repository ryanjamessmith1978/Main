// *** Modify Boat Mesh ***
// This ingenious script modifies how the mesh data is interpreted by Unity directly by taking the polygons on the bottom of the boat mesh and 
// finding their positions in relation to world space and then cutting the polygons in half to make triangles.  
// Triangles are then stored in two triangle containers.  One container has the triangle data position data, 
// while the second container has the order of the vertices that make up the triangle.
// This triangle data is used to detect what triangles are submerged under water.
// The script finally uses this data to then create a bounding box on the upper and lower part of the boat mesh.
// These invisible force box meshes are located from the center of point of where the empty child game objects of the player controller are placed
// in relation to the player in the Unity editor.  The physics forces of this game are applied to where these invisible meshes are created
// by Ryan J Smith
...
public class ModifyBoatMesh
{
    ...
    //Generate the underwater mesh (and the above water mesh)
    ...
    //Add all the triangles that's part of the underwater and above water meshes
    ...
    //Display the underwater or above water mesh
