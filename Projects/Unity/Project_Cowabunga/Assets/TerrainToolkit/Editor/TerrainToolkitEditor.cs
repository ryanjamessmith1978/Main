		...
		switch (terrain.toolModeInt) {
			...
			switch (terrain.generatorTypeInt) {
				...
				if (GUI.Button(buttonRect, "Generate Voronoi Features")) {
					...
					TerrainData teraData = terr.terrainData;
					Undo.RegisterUndo(teraData, "Terrain Generator");
				...
				if (GUI.Button(buttonRect, "Generate Fractal Terrain")) {
					...
					TerrainData torData = tera.terrainData;
					Undo.RegisterUndo(torData, "Terrain Generator");
					TerrainToolkit.GeneratorProgressDelegate generatorProgressDelegate = new TerrainToolkit.GeneratorProgressDelegate(updateGeneratorProgress);
					terrain.generateTerrain(generatorProgressDelegate);
					EditorUtility.ClearProgressBar();
					GUIUtility.ExitGUI();
				...
				if (GUI.Button(buttonRect, "Generate Perlin Terrain")) {
					...
					TerrainData teraaData = terrr.terrainData;
					Undo.RegisterUndo(teraaData, "Terrain Generator");
					TerrainToolkit.GeneratorProgressDelegate generatorProgressDelegate = new TerrainToolkit.GeneratorProgressDelegate(updateGeneratorProgress);
					terrain.generateTerrain(generatorProgressDelegate);
					EditorUtility.ClearProgressBar();
					GUIUtility.ExitGUI();
				...
				if (GUI.Button(buttonRect, "Smooth Terrain")) {
					// Undo...
					Terrain tere = (Terrain) terrain.GetComponent(typeof(Terrain));
					if (tere == null) {
						return;
					}
					TerrainData tereData = tere.terrainData;
					Undo.RegisterUndo(tereData, "Smooth Terrain");
					TerrainToolkit.GeneratorProgressDelegate generatorProgressDelegate = new TerrainToolkit.GeneratorProgressDelegate(updateGeneratorProgress);
					terrain.generateTerrain(generatorProgressDelegate);
					EditorUtility.ClearProgressBar();
					GUIUtility.ExitGUI();
				...
				if (GUI.Button(buttonRect, "Normalise Terrain")) {
					// Undo...
					Terrain teer = (Terrain) terrain.GetComponent(typeof(Terrain));
					if (teer == null) {
						return;
					}
					TerrainData teerData = teer.terrainData;
					Undo.RegisterUndo(teerData, "Normalise Terrain");
					TerrainToolkit.GeneratorProgressDelegate generatorProgressDelegate = new TerrainToolkit.GeneratorProgressDelegate(updateGeneratorProgress);
					terrain.generateTerrain(generatorProgressDelegate);
					EditorUtility.ClearProgressBar();
					GUIUtility.ExitGUI();
			...
			switch (terrain.erosionTypeInt) {
				...
				if (GUI.Button(buttonRect, "Apply thermal erosion")) {
					// Undo
					Terrain teraf = (Terrain) terrain.GetComponent(typeof(Terrain));
					if (teraf == null) {
						return;
					}
					TerrainData terafData = teraf.terrainData;
					Undo.RegisterUndo(terafData, "Terrain Erosion");
					// Start time...
				...
				if (GUI.Button(buttonRect, "Apply hydraulic erosion")) {
					// Undo
					Terrain teera = (Terrain) terrain.GetComponent(typeof(Terrain));
					if (teera == null) {
						return;
					}
					TerrainData teeraData = teera.terrainData;
					Undo.RegisterUndo(teeraData, "Terrain Erosion");
					// Start time...
					DateTime startTime = DateTime.Now;
					TerrainToolkit.ErosionProgressDelegate erosionProgressDelegate = new TerrainToolkit.ErosionProgressDelegate(updateErosionProgress);
					terrain.erodeAllTerrain(erosionProgressDelegate);
				...
				if (GUI.Button(buttonRect, "Apply tidal erosion")) {
					// Undo
					Terrain ters = (Terrain) terrain.GetComponent(typeof(Terrain));
					if (ters == null) {
						return;
					}
					TerrainData tersData = ters.terrainData;
					Undo.RegisterUndo(tersData, "Terrain Erosion");
					// Start time...
					DateTime startTime = DateTime.Now;
					TerrainToolkit.ErosionProgressDelegate erosionProgressDelegate = new TerrainToolkit.ErosionProgressDelegate(updateErosionProgress);
					terrain.erodeAllTerrain(erosionProgressDelegate);
				...
				if (GUI.Button(buttonRect, "Apply wind erosion")) {
					// Undo
					Terrain teraform = (Terrain) terrain.GetComponent(typeof(Terrain));
					if (teraform == null) {
						return;
					}
					TerrainData teraformData = teraform.terrainData;
					Undo.RegisterUndo(teraformData, "Terrain Erosion");
					// Start time...
					DateTime startTime = DateTime.Now;
					TerrainToolkit.ErosionProgressDelegate erosionProgressDelegate = new TerrainToolkit.ErosionProgressDelegate(updateErosionProgress);
					terrain.erodeAllTerrain(erosionProgressDelegate);
