...
public class PlayerController4 : MonoBehaviour {
    ...
    void FixedUpdate()
    {
        ...
        float jMoveVert4 = Input.GetAxis("VerticalController4");
        float jMoveHorz4 = Input.GetAxis("HorizontalController4");



        Vector3 jMove4 = new Vector3(jMoveHorz4, 0.0f, jMoveVert4);

        if (Input.GetButton("A4"))
            sprintburst = 3;
        else
            sprintburst = 1;
        ...
        // joystick movement
        if (Input.GetAxis("VerticalController4") > 0 || Input.GetAxis("HorizontalController4") > 0 || Input.GetAxis("VerticalController4") < 0 || Input.GetAxis("HorizontalController4") < 0)
            rb.AddForce(jMove4 * moveSpeed * sprintburst);
    ...
    public void OnTriggerEnter(Collider other)
    {
        ...
        if (other.gameObject.CompareTag("Sword1") || other.gameObject.CompareTag("Sword2") || other.gameObject.CompareTag("Sword3"))
        {
            ...
            GameObject.Find("Player4").GetComponent<PlayerController4>().enabled = false;
