...
function Player:checkLeftCollisions(dt)
    ...
    local tileTopLeft = self.map:pointToTile(self.x - 1.5, self.y + 3)
    local tileBottomLeft = self.map:pointToTile(self.x - 1.5, self.y + self.height - 1)
...
function Player:checkRightCollisions(dt)
    ...
    local tileTopRight = self.map:pointToTile(self.x + self.width - 1, self.y + 3)
...
function Player:checkObjectCollisions()
    ...
    for k, object in pairs(self.level.objects) do
        if object:collides(self) then
            if object.solid and object.consumable and self.hasKeyBlock == true and object.texture == 'flags' then
                object.onConsume(self)
                table.remove(self.level.objects, k)
                gStateMachine:change('play')
            elseif object.consumable and object.texture == 'keys' and object.frame == 2 then
                self.hasKey = true
            elseif object.solid and object.consumable and self.hasKey == true and object.texture == 'locks' then
                self.hasKeyBlock = true
                object.onConsume(self)
                table.remove(self.level.objects, k)
            elseif object.solid then
                table.insert(collidedObjects, object)
            elseif object.consumable and object.texture ~= 'flags' then
                object.onConsume(self)
                table.remove(self.level.objects, k)
            end
        end
