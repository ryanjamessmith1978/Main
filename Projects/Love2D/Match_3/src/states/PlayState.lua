...
function PlayState:enter(params)
    ...
    self.scoreGoal = self.level * 1.5 * 1500
end
...
function PlayState:update(dt)
    ...
    self.board:anyMatches()
    Timer.update(dt)
end

function PlayState:tileSwap(tile1, tile2, swapBack)
...
function PlayState:calculateMatches(calculatedMatches)
    ...
    if matches then
        ...
        bonusPoints = self.board.bonusMatches
        self.score = self.score + bonusPoints * 10

        -- remove any tiles that matched from the board, making empty spaces
        self.board:removeMatches()
        
        numTiles = self.board.removedTiles
        self.timer = self.timer + numTiles
        numTiles = 0
        ...
        Timer.tween(1.00, tilesToFall):finish(function()
...
function PlayState:render()
    ...
    if self.noMatches then
        love.graphics.setColor(125/255, 215/255, 200/255, 200/255)
        love.graphics.rectangle('fill', 0, self.noMatchLabelY - 8, VIRTUAL_WIDTH, 32)
        love.graphics.setColor(255, 255, 255, 255)
        love.graphics.setFont(gFonts['medium'])
        love.graphics.printf('No matches available, reloading', 0, self.noMatchLabelY, VIRTUAL_WIDTH, 'center')
    end
end